...
function CreateTemplateModal({ isOpen, onClose, onSave }) {
  ...
  return (
    <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center">
      <div className="bg-white rounded-xl p-6 w-full max-w-lg shadow-2xl text-black">
        ...
        <div className="mb-2">
          ...
          <div className="flex flex-col gap-2">
            ...
            <input
              ...
              onChange={e => setActivityForm(f => ({ ...f, duration: Number(e.target.value) }))}
            />
            <input
              className="border rounded p-2 text-black"
              placeholder="Category"
            ...
            />
            <TimeOfDaySelector
              value={activityForm.timeOfDay}
              onChange={v => setActivityForm(f => ({ ...f, timeOfDay: v }))}
            />
            <button
              className="mt-2 px-3 py-1 rounded bg-blue-100 text-blue-800"
              onClick={() => {
                setActivities(a => [...a, activityForm]);
                setActivityForm({ name: '', duration: 60, category: '', icon: '', location: '', timeOfDay: 'morning' });
              }}
            >
              Add Activity
            </button>
          ...
          <ul className="mb-2 text-black">
            {activities.map((a, i) => (
              <li key={i} className="text-sm flex gap-2 items-center">
                {a.icon} {a.name}
                <span className="text-xs text-gray-500">({a.timeOfDay})</span>
              </li>
            ))}
          </ul>
        </div>
