      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div className="bg-white rounded-xl shadow-lg p-6 mb-6" style={{ backgroundColor: 'var(--bg)', color: 'var(--text)' }}>
          <div className="flex justify-between items-center mb-4">
            <div>
              <h1 className="text-3xl font-bold">GoSplit</h1>
              <p>Split expenses with your travel groups</p>
            </div>
            <div className="flex space-x-3">
              <button onClick={refreshGroups} className="bg-gray-500 hover:bg-gray-600 text-white px-4 py-2 rounded-lg flex items-center space-x-2">
                <span>🔄</span><span>Refresh</span>
              </button>
              <button onClick={() => setShowJoinModal(true)} className="bg-green-500 hover:bg-green-600 text-white px-4 py-2 rounded-lg flex items-center space-x-2">
                <span>📨</span><span>Join Group</span>
              </button>
              <button onClick={() => setShowCreateModal(true)} className="bg-indigo-500 hover:bg-indigo-600 text-white px-4 py-2 rounded-lg flex items-center space-x-2">
        ...
        {groups.length === 0 ? (
          <div className="bg-white rounded-xl shadow-lg p-8 text-center" style={{ backgroundColor: 'var(--bg)', color: 'var(--text)' }}>
            ...
            <p className="mb-6">Create your first travel group or join an existing one to start splitting expenses!</p>
            <div className="flex justify-center space-x-4">
              <button onClick={() => setShowCreateModal(true)} className="bg-indigo-500 hover:bg-indigo-600 text-white px-6 py-3 rounded-lg">Create First Group</button>
              <button onClick={() => setShowJoinModal(true)} className="bg-green-500 hover:bg-green-600 text-white px-6 py-3 rounded-lg">Join a Group</button>
            </div>
          </div>
        ) : (
